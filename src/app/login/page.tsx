...
// Se o seu Header.tsx estiver em src/components/Header.tsx e este arquivo em src/app/login/page.tsx,
// o caminho relativo correto seria '../../components/Header'.
// Se você tiver uma estrutura de pastas diferente (ex: src/app/(auth)/login/page.tsx),
...
// import { Eye, EyeSlash, Check, ArrowRight } from '@phosphor-icons/react';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      // Usando window.location.href para navegação direta sem next/router
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verifica se a resposta da API não foi bem-sucedida (status 2xx)
      if (!response.ok) {
        const errorData = await response.json(); // Pega a mensagem de erro do corpo da resposta
        throw new Error(errorData.error || 'Erro ao fazer login.'); // Lança um erro com a mensagem da API ou uma genérica
      }

      // Se o login for bem-sucedido
      const data = await response.json();
      // Em um cenário real, você armazenaria um token JWT recebido da API
      // localStorage.setItem('token', data.token); // Exemplo: armazenar token
      setSuccess('Login bem-sucedido! Redirecionando...'); // Define mensagem de sucesso

      // Redireciona para a página principal após um pequeno atraso para o usuário ver a mensagem de sucesso
      setTimeout(() => {
        // Usando window.location.href para navegação direta sem next/router
        window.location.href = '/';
      }, 1500);
      ...
      // Captura e exibe erros de rede ou erros retornados pela API
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <main className="flex-grow p-6 flex justify-center items-center">
        ...
        <div className="w-full max-w-md space-y-6 bg-white p-8 rounded-2xl shadow-lg text-gray-900">
          ...
          {/* Título e subtítulo da seção de login */}
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">RooBus</h1>
            <h2 className="text-xl font-semibold text-gray-700 mb-1">Acesse sua Conta</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {isLoading ? (
                <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> // Ícone de carregamento
              ) : (
                <>
                  Entrar {/* Ícone de seta em SVG simplificado */}
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="text-center mt-6">
            <p className="text-gray-600">
              Não tem uma conta?{' '}
              <a
                href="/cadastro" // Usando <a> tag em vez de Link
          ...
          <div className="text-center mt-4">
            <a
              href="/" // Usando <a> tag em vez de Link
